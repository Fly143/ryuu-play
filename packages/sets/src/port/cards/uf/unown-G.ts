...
export class UnownG extends PokemonCard {
  ...
  public attacks: Attack[] = [
      { name: "Hidden Power", cost: [], damage: "", text: "Count the number of your Pokémon that have any damage counters on them. Put that many damage counters on the Defending Pokémon." }
  ];
  public set: string = "UF";
  public name: string = "Unown";
  public fullName: string = "Unown UF G";
  public text: string = "Unown";

  public reduceEffect(store: StoreLike, state: State, effect: Effect): State {
    if (effect instanceof AttackEffect && effect.attack === this.attacks[0]) {
      return commonEffects.runAttackOp(this, store, state, effect).use(effect, "putCountersOnDefendingPerSelf");
    }
